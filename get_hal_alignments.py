...
import time
import re
#sys.path.insert(0, '/exports/cmvm/datastore/sbms/groups/young-lab/caitlin/scripts/')
sys.path.insert(0, '/exports/cmvm/eddie/sbms/groups/young-lab/caitlin/scripts/')
from modules import *

from Bio.Seq import Seq

#cat */*/*bed | grep -v Chrom | grep -v start | awk '{print "chr"$1"\t"$2"\t"$3"\tchr"$1":"$2"-"$3}' | sort -k1,1 -k2,2n | uniq > ALL.bed

job_id = sys.argv[1] # This is the job id, now can accept jobs of 1 to 100

# Modify to perform job # 9 in 100 interals
# Now, going to do the shuffle - do job 3 to start with
#shuffleBed -i highlyConservedEnhancers_H3K27ac_placentals -g /exports/igmm/datastore/taylor-lab/genomes/hg19/hg19.genome > shuffle.highlyConservedEnhancers_H3K27ac_placentals
...
# mouse mm10 is C57B6J - check again
# grep -v size /exports/igmm/eddie/taylor-lab/genomes/mm10/mm10.genome | awk '{print $2}' | awk '{s+=$1} END {print s}' = 2730871774
#halStats /exports/igmm/eddie/taylor-lab/craig/lce/ref/1509_outgroups.hal | grep -v Anc | grep -v stitch | grep C57B6J
#C57B6J, 0, 2730871774, 66, 10609658, 0
# By following the same story, I found that rattus is rn5
# rheMac3 is definitely rheMac3 and not rheMac8

# CAROLI and Pahari removed on Craig's advice
# source /exports/igmm/eddie/taylor-lab/craig/lce/bin/progressiveCactus/environment
...
#species = ['C57B6J', 'hg19', 'rheMac3']
#species_names = ['mouse', 'human', 'macaque']

#[ryoung23@node2c15(eddie) hal_alignments]$ grep -v random /exports/igmm/eddie/taylor-lab/genomes/hg19/hg19.genome  | grep -v Un | grep -v hap | grep -v chrM > hg19.genome
#[ryoung23@node2c15(eddie) hal_alignments]$ shuffleBed -i ffd.bed -g hg19.genome > ffd_shuffle.bed
#[ryoung23@node2c15(eddie) hal_alignments]$ shuffleBed -i second.bed -g hg19.genome > second_shuffle.bed

# Rerun to add ffd.bed and second.bed
#promoter_infile = ls_list('*bed')[(int(job_id) - 1)]
#promoter_infile = ls_list('ANDNOT.bed')[0]
promoter_infile = ls_list('ALL.bed')[0]
infile_type = promoter_infile.split('.')[0]
genome = 'hg19'
genome_dir = genome
output_dir = genome

if 'shuffle' in infile_type:
#	genome_dir = promoter_infile.split('.promoters')[0]
#	genome = genome_dir.split('_')[0]

	output_dir = genome + '_shuffle'
...
#slopBed = 'slopBed -i ' + list_infile + ' -g /exports/igmm/eddie/taylor-lab/genomes/hg19/hg19.genome -b 5000 > ' + list_infile + '.2'
#print slopBed
#call(slopBed, shell=True)
...
if correct_compressed == 0:
	...
	if os.path.exists(projection_outfile):
		...
		for line in infile:
			if '>' in line:
#				print line
				id = line.split(':')[1] + ':' + line.split(':')[2] + ':' + line.split(':')[3]
...
#	exit()
		...
		hal2maf = '/exports/cmvm/eddie/sbms/groups/young-lab/rob/scripts/hal/hal2maf /exports/cmvm/eddie/sbms/groups/young-lab/genomes/1509_outgroups.hal temp/' + job_id + '.maf --maxRefGap ' + str(length) + ' --refGenome ' + hal_genome + ' --targetGenomes ' + str(','.join(species)) + ' --refTargets temp/' + job_id + '_maf.bed'
